...
def visualize_attention_matrix(attention_matrix, sequence, tokenizer):
    ...
    masked_attention_matrix = np.mean(np.array(masked_attention_matrix.cpu().detach().numpy()), axis=(0))# average over heads within each layer
    masked_attention_matrix = np.clip((masked_attention_matrix - masked_attention_matrix.min()) / (masked_attention_matrix.max() - masked_attention_matrix.min()), 0)
